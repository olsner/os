...
GRUBDIR      := $(OUTDIR)/grub
ASMDIR       := kasm
MOD_ASMFILES := user/newproc.asm user/loop.asm user/shell.asm
...
MOD_CFILES   += cuser/ioapic.c cuser/pic.c cuser/irq.c cuser/console.c
...
# deadlocks.
ELFS_WANT_LIBC_RAW := \
	$(OUTDIR)/cuser/irq.elf \
	$(OUTDIR)/cuser/pic.elf \
	$(OUTDIR)/cuser/apic.elf \
	$(OUTDIR)/cuser/ioapic.elf \
	$(OUTDIR)/cuser/acpica.elf \
	$(OUTDIR)/cuser/console.elf
