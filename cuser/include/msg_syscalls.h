...
enum msg_tx_flags {
	...
	MSG_TX_ACCEPTFD = 1ULL << 63,
	// Page-fault transaction, response must be sent with grant(). Must not be set in an outgoing message.
	MSG_TX_PFAULT = 1ULL << 62,
	// When sending a response with the MSG_TX_FD flag, also close the file
	// descriptor as soon as it's been sent to the recipient.
	MSG_TX_CLOSEFD = 1ULL << 61,
};
...
static inline ipc_dest_t msg_mkdest(int32_t fd, enum msg_tx_flags flags) {
	return flags | (uint64_t)fd;
}
// Get the file descriptor field from an IPC destination.
static inline int msg_dest_fd(ipc_dest_t dest) {
